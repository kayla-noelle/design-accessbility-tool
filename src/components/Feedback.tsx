...
export function Feedback({ backgroundColor, headingColor, bodyColor, uiColor,btnTextColor }: FeedbackProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-white rounded-xl shadow">
      <p className="text-[20px]">Heading contrast: {headingContrast.toFixed(2)} — {ratingText(headingContrast)}</p>
      <p className="text-[20px]">Body text contrast: {bodyContrast.toFixed(2)} — {ratingText(bodyContrast)}</p>
      <p className="text-[20px]">Button contrast: {buttonContrast.toFixed(2)} — {ratingText(buttonContrast)}</p>
      <p className="text-[20px]">Button text contrast: {buttonTextContast.toFixed(2)} — {ratingText(buttonTextContast)}</p>
    </div>
  );
}
